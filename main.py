...
# Paste everything from STATUS_MAP down to build_match_info()
# (Same as before — no changes needed in logic)

app = FastAPI(
    title="YSScores Match API",
    description="Fetch live or scheduled football match data by match ID from YSScores.com",
    version="1.0"
)
...
def get_match_data(match_id: str):
    ...
    teams = soup_page.select(".team-item")
    if len(teams) < 2:
        raise Exception("Could not find team info")
...
def match_endpoint(match_id: str):
    if not match_id.isdigit():
        raise HTTPException(status_code=400, detail="Invalid match ID. Must be numeric.")
    try:
...
# This makes FastAPI work on WSGI servers (like PythonAnywhere free tier)
application = app